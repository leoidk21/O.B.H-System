...
type ExpenseStatus = "Pending" | "Proof Submitted" | "Paid" | "Verified" | "Rejected";
...
const Budget: React.FC = () => {
  ...
  const pickProofForExpense = async (expenseId: string) => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      setExpenses(prev =>
        prev.map(e => (e.id === expenseId ? { ...e, proofUri: uri, status: "Proof Submitted" } : e))
      );

      try {
        await uploadProofToServer(expenseId, uri);
      } catch (err) {
        Alert.alert("Upload failed", "Couldn't upload proof. Proof is kept locally for now.");
      }
    }
  };
